...
$apimanagementname="vivethereapimnginstance1129" #+$d.Year+$d.Month+$d.Day+$d.Hour+$d.Minute+$d.Second+$d.Millisecond
$apim_service_url="https://"+$apimanagementname+".azure-api.net/"+$apimngpath+"/"
$publicapiendpoint="https://"+$apimanagementname+".azure-api.net/"+$apimngpath+"/api/"
...
#az webapp cors add -g $groupname -n $appserviceapiname --allowed-origins "*"

#create api management instance
az apim create --name $apimanagementname --resource-group $groupname -l $locationapi `
               --publisher-email "vadym.tarasov@gmail.com" --publisher-name  "Pupkin Inc" --sku-name "Consumption"
az apim api create --service-name $apimanagementname -g $groupname `
                   --api-id "vivetherepublicapi" --display-name "vivetherepublicapi" --path $apimngpath `
                   --protocols https --service-url $apim_service_url
