...
cd "C:\Users\Vadym\source\repos\eShopOnWeb"

$alias="vivthre"

$groupname=$alias+"rg"
$grouplocation="eastus2"
...
$acr_name="vadtarasacr"
az acr create --name $acr_name --resource-group $groupname --sku Standard --admin-enabled true #use az acr update

az acr build --registry $acr_name  --resource-group $groupname --image vivetherepublicapi:v1 -f ./src/publicapi/Dockerfile_elm8 .
#Get admin user name
$acradmin=$(az acr credential show --name $acr_name  --resource-group $groupname --query username -o tsv)
echo $acradmin
#Get admin password
$acradminpassword=$(az acr credential show --name $acr_name  --resource-group $groupname --query passwords[0].value -o tsv)
echo $acradminpassword
#get image name
$acrImageName=$acr_name+".azurecr.io/vivetherepublicapi:v1"

#Create appservice plan Linux and service app for API
az appservice plan create --name $plannameapi --sku "S1" --resource-group $groupname --location $locationapi --is-linux
az webapp create --name $appserviceapiname --resource-group $groupname --plan $plannameapi `
				 --assign-identity "[system]" `
				 --deployment-container-image-name $acrImageName `
				 --docker-registry-server-user $acradmin `
				 --docker-registry-server-password $acradminpassword
...
#For container use appsettings
az webapp config appsettings set --name $appserviceapiname -g $groupname --settings CatalogConnection=$catalogConnection
az webapp config appsettings set --name $appserviceapiname -g $groupname --settings IdentityConnection=$catalogConnection
...
$warehouseConnectionString=$(az storage account show-connection-string --name $dataStorageAccountName -g $groupname --query connectionString -o tsv)
...
$deliveryFuncUrl = "https://"+$deliveryDefaultHost+"/api/"
...
az functionapp config appsettings set -n $warehouseFuncAppName -g $groupname --settings ServiceBusConnectionString=$serviceBusConnString
az functionapp config appsettings set -n $warehouseFuncAppName -g $groupname --settings WarehouseStorageConnectionString=$warehouseConnectionString
